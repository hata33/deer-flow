...
**关键设计**：`asyncio.create_task` 把 `run_agent` 注册到事件循环但不立即执行。此时同一个循环上挂着两个协程——`run_agent`（等调度）和 `sse_consumer`（等队列数据）——它们通过 `StreamBridge` 解耦。`StreamBridge` 是项目自实现的发布-订阅桥接层（`deerflow/runtime/stream_bridge/`），底层基于 `asyncio.Queue` 按 `run_id` 分发事件：`run_agent` 调用 `bridge.publish()` 写入，`sse_consumer` 通过 `bridge.subscribe()` 消费。
