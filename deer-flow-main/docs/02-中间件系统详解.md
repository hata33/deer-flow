...
graph TB
    ...
    subgraph "后置处理"
        ...
        P1 --> P2[SandboxMiddleware<br/>释放沙箱]
        P2 --> P3[SandboxAuditMiddleware<br/>沙箱审计]
        P3 --> P4[响应返回]
    end
...
| 中间件文件 | 位置 | 职责描述 | 执行时机 |
|-----------|------|---------|---------|
| thread_data_middleware.py | agents/middlewares/ | 线程数据初始化和管理 | 前置 |
| uploads_middleware.py | agents/middlewares/ | 文件上传处理和路径注入 | 前置 |
| sandbox_middleware.py | sandbox/middleware.py | 沙箱环境准备 | 前置+后置 |
| dangling_tool_call_middleware.py | agents/middlewares/ | 悬挂工具调用检测和修复 | 前置 |
| tool_error_handling_middleware.py | agents/middlewares/ | 工具执行错误捕获和转换 | 工具调用 |
| title_middleware.py | agents/middlewares/ | 自动生成对话标题 | 后置 |
| memory_middleware.py | agents/middlewares/ | 长期记忆管理和更新 | 前置+后置 |
| view_image_middleware.py | agents/middlewares/ | 图像查看能力注入 | 前置 |
| subagent_limit_middleware.py | agents/middlewares/ | 子代理调用限制 | 前置 |
| loop_detection_middleware.py | agents/middlewares/ | 循环检测和强制停止 | 后置 |
| clarification_middleware.py | agents/middlewares/ | 澄清请求处理 | 前置+后置 |
| todo_middleware.py | agents/middlewares/ | 任务列表管理 | 前置 |
| sandbox_audit_middleware.py | agents/middlewares/ | 沙箱操作审计 | 后置 |
| token_usage_middleware.py | agents/middlewares/ | Token使用统计 | 后置 |
| deferred_tool_filter_middleware.py | agents/middlewares/ | 延迟工具过滤 | 前置 |
| guardrail_middleware.py | guardrails/middleware.py | 防护栏检查 | 工具调用 |
